...
export default function ProductDetail() {
  ...
  if (!producto) return (
    <div style={{ textAlign: 'center', padding: '4rem' }}>
      ...
      <button onClick={() => navigate('/')} style={{ marginTop: 12, padding: '8px 20px', background: 'var(--verde)', color: '#fff', borderRadius: 8 }}>
        Volver al inicio
      </button>
    </div>
  );
  ...
  const supermercadosEnHistorial = [...new Set(historial.map(h => h.supermercado))];
  ...
  const chartData = fechasUnicas.map(fecha => {
    const punto = { fecha };
    supermercadosEnHistorial.forEach(sup => {
      const entrada = historial.find(h =>
        new Date(h.fecha).toLocaleDateString('es-EC', { day: '2-digit', month: 'short' }) === fecha &&
        h.supermercado === sup
      );
      if (entrada) punto[sup] = Number(entrada.precio);
    });
    return punto;
  });
  ...
  const handleAddAlert = async () => {
    ...
    try {
      await alertsAPI.create({ producto_id: id, descuento_minimo: 15 });
  ...
  return (
    <div className="animate-fade">
      ...
      <button onClick={() => navigate(-1)} style={{
        ...
        color: 'var(--gris-400)', fontSize: 14, marginBottom: '1.5rem',
        background: 'none', padding: '4px 8px', borderRadius: 6
      }}>
        ← Volver
      </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1.5rem', alignItems: 'start' }}>
        ...
        <div>
          <div style={{
            ...
            textAlign: 'center', fontSize: 80, border: '1px solid var(--gris-200)',
            marginBottom: 12
          }}>
          ...
          <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', padding: '1.25rem', border: '1px solid var(--gris-200)' }}>
            ...
            <h1 style={{ fontSize: 20, fontWeight: 800, color: 'var(--gris-900)', marginBottom: 12 }}>{producto.nombre}</h1>
            ...
            <button
              ...
              style={{
                ...
                background: added ? 'var(--verde)' : 'var(--verde-claro)',
                color: added ? '#fff' : 'var(--verde)',
                fontSize: 15, fontWeight: 700, transition: 'all 0.2s', marginBottom: 8
              }}>
              {added ? '✓ Agregado' : '+ Agregar al carrito'}
            </button>
            <button onClick={handleAddAlert} style={{
              width: '100%', padding: '9px', borderRadius: 10,
              background: alertCreated ? '#fff7ed' : '#fff',
              color: alertCreated ? '#c2410c' : 'var(--gris-600)',
              fontSize: 13, fontWeight: 500, border: '1px solid var(--gris-200)'
            }}>
              {alertCreated ? '🔔 Alerta creada' : '🔔 Alertar cuando baje'}
            </button>
          </div>
        </div>

        {/* Col derecha: comparador */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {/* Resumen precios */}
          {ahorro > 0.05 && (
            <div style={{
              background: 'linear-gradient(135deg, var(--verde-claro), #d1fae5)',
              borderRadius: 'var(--radius-lg)', padding: '1.25rem',
              border: '1px solid #86efac', display: 'flex', alignItems: 'center', gap: 16
            }}>
              <div style={{ fontSize: 40 }}>💰</div>
              <div>
                <div style={{ fontSize: 13, color: 'var(--verde-hover)', fontWeight: 600 }}>Ahorro potencial</div>
                <div style={{ fontSize: 28, fontWeight: 800, color: 'var(--verde)', fontFamily: 'var(--font-display)' }}>
                  ${ahorro}
                </div>
                <div style={{ fontSize: 13, color: 'var(--gris-600)' }}>
                  entre {SUP_LABELS[masBarato?.supermercado]} y {SUP_LABELS[masCaro?.supermercado]}
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', border: '1px solid var(--gris-200)', overflow: 'hidden' }}>
            <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid var(--gris-100)', fontWeight: 700, fontSize: 15 }}>
              Comparar precios
            </div>
            {precios.map((p, i) => (
              <div key={p.supermercado} style={{
                display: 'flex', alignItems: 'center', padding: '14px 1.25rem',
                borderBottom: i < precios.length - 1 ? '1px solid var(--gris-100)' : 'none',
                background: i === 0 ? '#f0fdf4' : '#fff',
                gap: 14
              }}>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: SUP_COLORS[p.supermercado], flexShrink: 0 }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--gris-800)' }}>{SUP_LABELS[p.supermercado]}</div>
                  {p.precio_anterior && (
                    <div style={{ fontSize: 12, color: 'var(--gris-400)' }}>
                      Antes: ${Number(p.precio_anterior).toFixed(2)}
                    </div>
                  )}
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 22, fontWeight: 800, color: i === 0 ? 'var(--verde)' : 'var(--gris-700)', fontFamily: 'var(--font-display)' }}>
                    ${Number(p.precio).toFixed(2)}
                  </div>
                  {p.descuento_porcentaje > 0 && (
                    <span style={{ fontSize: 11, background: 'var(--rojo)', color: '#fff', padding: '2px 6px', borderRadius: 4, fontWeight: 700 }}>
                      -{Math.round(p.descuento_porcentaje)}%
                    </span>
                  )}
                </div>
                {i === 0 && <span className="badge-verde">Más barato</span>}
                {i > 0 && (
                  <span style={{ fontSize: 11, color: 'var(--gris-400)', minWidth: 60, textAlign: 'right' }}>
                    +${(p.precio - masBarato.precio).toFixed(2)}
                  </span>
                )}
              </div>
            ))}
            ...
            <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', border: '1px solid var(--gris-200)', padding: '1.25rem' }}>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                  ...
                  {supermercadosEnHistorial.map(sup => (
                    <Line key={sup} type="monotone" dataKey={sup} stroke={SUP_COLORS[sup]} strokeWidth={2}
...
function getCategoryEmoji(categoria) {
  const map = {
    ...
    'Azúcares': '🍬', 'Panadería': '🍞', 'Huevos': '🥚', 'Conservas': '🐟',
    'Limpieza del Hogar': '🧹', 'Cuidado Personal': '🧴', 'Bebidas': '🥤',
    'Pastas y Fideos': '🍝', 'Condimentos': '🧂', 'Higiene': '🧻', 'Carnes y Aves': '🍗',
    'Frutas y Verduras': '🥦', 'Snacks': '🍿'
  };
